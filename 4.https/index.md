...
- TLS/SSL 的功能实现主要依赖于三类基本算法：**散列函数** 、**对称加密**和**非对称加密**，其利用`非对称加密`实现身份认证和密钥协商，`对称加密算法`采用协商的密钥对数据加密，基于`散列函数`验证信息的完整性。
