...
- 此外，TCP 作为一种面向`有连接`的协议，只有在`确认通信`对端存在时才会发送数据，从而可以控制通信流量的浪费。
...
如果只需要两次握手，中间人攻击者可以伪造第二次握手（SYN+ACK），使服务器认为连接已建立，但实际上客户端并未收到这个响应。通过这种方式，攻击者可以中断连接，或者在客户端和服务器之间插入自己，造成安全问题。
